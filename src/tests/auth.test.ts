...
describe("Auth Tests", () => {
    ...
    test("Auth test registration", async () => {
      ...
      const response2 = await request(app).post("/posts").set({authorization: "JWT " + testUser.accessToken}).send(
    {
        "title": "test title",
        "content": "test content",
        "owner": testUser._id
      ...
      test("Refresh token multiple times", async () => {
        ...
        const response4 = await request(app).post(baseUrl + "/refresh").send({refreshToken: newRefreshToken});
      ...
      test("Refresh token timeout", async () => {
        ...
        const response2 = await request(app).post("/posts").set({authorization: "JWT " + testUser.accessToken}).send({title: "test title", content: "test content",owner: testUser._id});
        expect(response2.statusCode).toBe(401);
        const response3 = await request(app).post(baseUrl + "/refresh").send({refreshToken: testUser.refreshToken});
        expect(response3.statusCode).toBe(200);
        testUser.accessToken = response3.body.accessToken;
        testUser.refreshToken = response3.body.refreshToken;
        const response4 = await request(app).post("/posts").set({authorization: "JWT " + testUser.accessToken}).send({title: "test title", content: "test content",owner: testUser._id});
